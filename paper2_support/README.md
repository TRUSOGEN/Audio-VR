...
上传快照：2026-09-25 19:40 AEST（Australia/Sydney）。这份目录从 Mac 上的 `/Users/trusoegn/论文2/` 复制；每个文件的源修改时间、大小与 SHA-256 见 [`source_manifest_20260925.csv`](source_manifest_20260925.csv)。Git 不保存文件修改时间，因此以该清单追溯时间，不能把 GitHub 上传日当成声音定稿日。
...
先在 Windows Unity 运行界面或 `session_manifest.json` 确认实际 `AudioV0` 数据根目录，目录下应有 `technical_demo`、`pilot` 或 `participant_run` 子目录。未改产品名时，常见位置是 `%USERPROFILE%\AppData\LocalLow\DefaultCompany\Audio V0\AudioV0`；实际路径以本机为准。Python 需要 3.9 或更新版本；图件导出另需 `matplotlib`。

在命令提示符中，从仓库根目录运行：

```bat
paper2_support\tools\start_data_viewer_windows.cmd "C:\实际路径\AudioV0"
```

启动后在浏览器打开 `http://127.0.0.1:8091/`。服务只监听本机并只读已有 JSONL；类别分开审阅。正式 CSV 导出使用：
