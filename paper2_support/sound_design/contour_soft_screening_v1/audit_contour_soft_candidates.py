"""Perform a digital-signal audit of contour-soft screening candidates.

The audit validates file format, duration, peak, RMS, and checksums. It does
not validate playback loudness, audibility, annoyance, comfort, or usability.
"""
...
def audit_wav(path: Path) -> dict[str, object]:
    """Return reproducible digital-signal metadata for one generated WAV file."""
    ...
    return {
        ...
        "rms_digital_amplitude": round(
            math.sqrt(sum(sample * sample for sample in samples) / len(samples)), 6
        ),
    }


def main() -> None:
    """Validate all C0--C3 files and emit a machine-readable audit record."""
    results = [audit_wav(path) for path in sorted(ROOT.glob("C*/*.wav"))]
    if len(results) != 12:
        raise ValueError(f"Expected 12 screening WAV files, found {len(results)}")
    errors = [
        result["file"]
